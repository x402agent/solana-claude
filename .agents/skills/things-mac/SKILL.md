---
name: things-mac
description: Manage Things 3 via the `things` CLI on macOS (add/update projects+todos via URL scheme; read/search/list from the local Things database). Use when a user asks SolanaOS to add a task to Things, list inbox/today/upcoming, search tasks, or inspect projects/areas/tags.
...
- If DB reads fail: grant **Full Disk Access** to the calling app (Terminal for manual runs; `SolanaOS.app` for gateway runs).

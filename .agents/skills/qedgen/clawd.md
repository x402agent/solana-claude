...
This file provides guidance to Clawd Code (clawd.ai/code) when working with code in this repository.

## Overview

QEDGen is a Clawd Code skill for formally verifying Solana programs using Lean 4 proofs. Clawd (the local LLM) drives proof writing directly — reading code, writing Lean models/theorems/proofs, and iterating on `lake build` errors. Leanstral (Mistral's theorem prover) is called only for hard sub-goals via `fill-sorry`.

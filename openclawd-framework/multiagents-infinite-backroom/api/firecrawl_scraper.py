...
Fetched content is injected directly into the agent conversation as context.
"""
...
# In-memory store: job_id -> {status, data, error, injected_at}
...
# ──────────────────────────────────────────────────────────────────────────────
# Backroom-specific: crawl dreams site and build agent context
# ──────────────────────────────────────────────────────────────────────────────
...
def _extract_context_block(pages: list[dict], max_chars: int = 12000) -> str:
    ...
    for page in pages:
        ...
        src = (page.get("metadata") or {}).get("sourceURL", "")
        title = (page.get("metadata") or {}).get("title", "")
        if not md.strip():
            continue
        header = f"### {title or src}\n_Source: {src}_\n\n"
        chunk = header + md.strip()
...
def crawl_dreams_and_inject(terminal, limit: int = 30) -> dict:
    """
    Crawl the Dreams of an Electric Mind site in real-time,
    then inject the content into the agent terminal's conversation context.
    Returns a summary dict with job info and injected character count.
    """

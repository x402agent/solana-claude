  ...
  return [
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        await ctx.runtime.writeFile(filePath, args.content as string);
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        return await ctx.runtime.readFile(args.path as string);
      },
    },
    {
      name: "expose_port",
...
========================`;
    ...
    },
    {
      name: "heartbeat_ping",
      description:
        "Publish a heartbeat status ping to CLAWD Runtime. Shows the world you are alive.",
      category: "survival",
      parameters: { type: "object", properties: {} },
      execute: async (_args, ctx) => {
        const credits = await ctx.runtime.getCreditsBalance();
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        return await gitDiff(ctx.runtime, repoPath, (args.staged as boolean) || false);
      },
    },
    {
      name: "git_commit",
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        return await gitCommit(ctx.runtime, repoPath, args.message as string, args.add_all !== false);
      },
    },
    {
      name: "git_log",
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        const entries = await gitLog(ctx.runtime, repoPath, (args.limit as number) || 10);
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        return await checkChildStatus(ctx.runtime, ctx.db, args.child_id as string);
      },
    },
    ...
    // ── Convex CLAWD Tools ──
    {
      ...
      description:
        "Store key-value data in the CLAWD Convex backend. Use this to save state, memories, or any data you want to persist across restarts. Data is stored per agent and can be tagged. Requires convexSiteUrl to be configured.",
      category: "survival",
      parameters: {
        type: "object",
        properties: {
      ...
      execute: async (args, ctx) => {
        if (!ctx.convex) return "Convex not configured. Set convexSiteUrl in config to enable.";
    ...
    },
    {
      ...
      description:
        "Retrieve data from the CLAWD Convex backend by key. Use this to recall previously stored state, memories, or configuration.",
      category: "survival",
      parameters: {
        type: "object",
        properties: {
      ...
      execute: async (args, ctx) => {
        ...
        if (!result) return `No data found for key="${args.key}"`;
    ...
    },
    {
      ...
      description:
        "List all stored data keys for your agent in the CLAWD Convex backend. Shows key names, content types, and value previews.",
      category: "survival",
      parameters: {
        type: "object",
        properties: {},
      ...
      execute: async (args, ctx) => {
        ...
        if (entries.length === 0) return "No stored data found.";
        return entries.map((e) => 
          `[${e.key}] ${e.contentType || 'text/plain'} — "${e.valuePreview.slice(0, 100)}${e.valuePreview.length > 100 ? '...' : ''}"`
        ).join('\n');
      },
    },
    ...
    // ── Model Discovery ──
    {
      name: "list_models",
      description:
        "List all available inference models from the CLAWD Runtime API with their provider and pricing. Use this to discover what models you can use and pick the best one for your needs.",
      category: "runtime",
      parameters: {
        type: "object",
        properties: {},

...
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | Default agent backend | Required for the primary runtime via `api/agents.py` |
| `DEEPSEEK_BASE_URL` | DeepSeek endpoint | Defaults to `https://api.deepseek.com` |
| `DEEPSEEK_MODEL` | Default DeepSeek model | Defaults to `deepseek-v4-pro` |
| `AGENT_BACKEND` | Agent backend selection | Defaults to `deepseek`; ignored while `DEEPSEEK_API_KEY` is set unless `DEEPSEEK_ALWAYS_DEFAULT=false` |
| `DEEPSEEK_ALWAYS_DEFAULT` | Backend priority guard | Defaults to `true`, so stale `AGENT_BACKEND` values cannot override DeepSeek |
| `PUMP_CLAWD_DARK_PROVIDER` | Pump critic provider | Defaults to `deepseek`; set `moonshot` to use Kimi when funded |
| `MOONSHOT_API_KEY` | Optional hosted critic/backend | Used by `api/moonshot_agents.py` and optional Dark Clawd routing |
| `OLLAMA_MODEL` | Optional local Ollama model | Defaults to `nemotron3:33b` when `AGENT_BACKEND=ollama` |
| `OLLAMA_HOST` | Optional local Ollama server | Defaults to `http://127.0.0.1:11434` |
| `OLLAMA_NUM_PREDICT` | Optional local response length | Defaults to `2048` |
| `OLLAMA_NUM_CTX` | Optional local context window | Defaults to `8192` |
...
| `PUMP_CONVEX_URL` | Pump.fun Convex deployment | Dedicated Pump data deployment, e.g. `https://artful-marten-138.convex.cloud` |
| `PUMP_CONVEX_ACTIONS_URL` | Pump.fun Convex HTTP actions | Dedicated Pump HTTP action site, e.g. `https://artful-marten-138.convex.site` |
| `PUMP_CONVEX_INGEST_SECRET` | Pump.fun Convex writes | Optional shared bearer for `/pump/*` Convex writes; set the same value in Convex env |
...
The API defaults to DeepSeek using the OpenAI-compatible API:
...
Local Ollama remains available only after disabling the DeepSeek priority guard:

```bash
ollama serve
ollama pull nemotron3:33b
DEEPSEEK_ALWAYS_DEFAULT=false AGENT_BACKEND=ollama OLLAMA_MODEL=nemotron3:33b npm run server
```

Then verify the backroom API:

```bash
curl -sS http://127.0.0.1:8000/healthz | jq .
curl -sS "http://127.0.0.1:8000/enter?message=hello"
```
...
| `/feed/channels` | GET | Available unified data feed channels |
| `/feed/snapshot` | GET | One-shot unified JSON snapshot for agents and relays |
| `/feed/ws` | WS | Unified websocket feed for conversation, agents, perps, pumpfun, stocks, and status |
| `/feed/push` | POST | Protected webhook push of one feed snapshot |
| `/feed/agent-setup.md` | GET | Markdown setup handoff for downstream agents |
...
| `/pump` | GET | Featured Pump.fun intelligence console |
| `/pumpfun/summary` | GET | Fast launch velocity, quality tiers, and relay health |
| `/pumpfun/tokens` | GET | Filterable scored token launch tape |
| `/pumpfun/intel` | GET | Premium scored Pump.fun intelligence feed |
| `/pumpfun/stream` | GET | Realtime Pump.fun SSE mirror |
| `/pumpfun/pay-quickstart.md` | GET | Pay gateway quickstart for Pump.fun stream agents |
| `/pumpfun/clawd/status` | GET | DeepSeek Clawd / Dark Clawd provider readiness |
| `/pumpfun/clawd/analyze` | POST | Paid DeepSeek-first live launch analysis |
| `/pumpfun/convex/status` | GET | Pump.fun Convex persistence readiness |
| `/pumpfun/convex/sync` | POST | Backfill recent Pump.fun launches into Pump Convex |

DeepSeek Clawd / Dark Clawd handoff instructions live in `docs/pump-clawd-agents.md`.
Pay gateway quickstart instructions live in `docs/pump-stream-pay-quickstart.md`.
...
Convex powers realtime presence, aiTown-style state, backroom messages, API-key verification, HTTP actions, scheduled market-data polling, and the dedicated Pump.fun launch/analysis archive.
...
| `pump.ts`, `pump/` | Pump.fun launch tape, AI analysis archive, and HTTP ingestion routes |
...
`pay/clawd-backroom-pay.yaml` defines x402/pay.sh metering for health, metadata, agent, loop, chat, Pump.fun intelligence, and Clawd analysis routes.
`pay/pump-stream-pay.yaml` is the focused sandbox quickstart spec for the Pump.fun SSE stream, paid launch tape, DeepSeek analysis, and Convex sync.
...
flyctl secrets set DEEPSEEK_API_KEY=... -a clawd-backrooms
flyctl secrets set MOONSHOT_API_KEY=... -a clawd-backrooms # optional
```

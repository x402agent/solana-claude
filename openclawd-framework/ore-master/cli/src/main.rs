...
async fn reset(
    ...
    let _config = get_config(rpc).await?;
...
async fn automate(
    ...
    let amount = std::env::var("AMOUNT").expect("Missing AMOUNT env var");
    let amount = u64::from_str(&amount).expect("Invalid AMOUNT");
    let deposit = std::env::var("DEPOSIT").unwrap_or("0".into());
    let deposit = u64::from_str(&deposit).expect("Invalid DEPOSIT");
    let executor =
        std::env::var("EXECUTOR").unwrap_or(ore_api::consts::EXECUTOR_ADDRESS.to_string());
    let executor = Pubkey::from_str(&executor).expect("Invalid EXECUTOR");
    let fee = std::env::var("FEE").unwrap_or("0".into());
    let fee = u64::from_str(&fee).expect("Invalid FEE");
    let mask = std::env::var("MASK").unwrap_or("1".into());
    let mask = u64::from_str(&mask).expect("Invalid MASK");
    let strategy = std::env::var("STRATEGY").unwrap_or("0".into());
    let strategy = u8::from_str(&strategy).expect("Invalid STRATEGY");
    let reload = std::env::var("RELOAD").unwrap_or("false".into());
    let reload = matches!(reload.as_str(), "1" | "true" | "TRUE" | "yes" | "YES");

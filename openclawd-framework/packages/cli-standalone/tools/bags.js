...
export class BagsTool {
    ...
    requireKey() {
        if (!this.apiKey)
            return { success: false, error: "BAGS_API_KEY not set" };
        return null;
    }
    async launchToken(args) {
        ...
        if (!kp)
            return { success: false, error: "Wallet not configured (SOLANA_PRIVATE_KEY)" };
        try {
            ...
            return wrap(e, "bags launchToken");
        }
    }
    async claimFees(positionKey) {
        const miss = this.requireKey();
        if (miss)
            return miss;
        const kp = this.wallet.getKeypair();
    ...
    async listPositions() {
        ...
        try {
            ...
            return wrap(e, "bags listPositions");
        }
    }
}

...
function wrap(err, prefix) {
    if (isAxiosError(err)) {
        if (err.code === "ECONNABORTED")
            return { success: false, error: `${prefix}: request timeout` };
        if (err.response)
...
export class SolanaTool {
    ...
    requireBirdeye() {
        if (!this.birdeyeApiKey)
            return { success: false, error: "BIRDEYE_API_KEY environment variable is not set" };
        return null;
    }
    async birdeyeGet(path, params, chain) {
    ...
    async getAsset(assetId) {
        if (!assetId || !BASE58.test(assetId))
            return { success: false, error: "Invalid Solana address" };
        if (!this.heliusApiKey)
            return { success: false, error: "HELIUS_API_KEY environment variable is not set" };
        try {
            const resp = await axios.post(`${this.heliusRpcUrl}?api-key=${this.heliusApiKey}`, { jsonrpc: "2.0", id: "1", method: "getAsset", params: { id: assetId } }, { timeout: 10000 });
            if (resp.data.error)
                return { success: false, error: `Helius: ${JSON.stringify(resp.data.error)}` };
    ...
    async getWalletBalance(walletAddress) {
        ...
        try {
            ...
            if (balance.data.error)
                return { success: false, error: `RPC: ${JSON.stringify(balance.data.error)}` };
            const lamports = balance.data.result.value;
            const accounts = (tokens.data.result?.value || []).map((a) => ({
                pubkey: a.pubkey,
    ...
    async getWalletPortfolio(walletAddress, chain = "solana") {
        if (!BASE58.test(walletAddress) && chain === "solana")
            return { success: false, error: "Invalid wallet address" };

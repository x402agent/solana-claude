...
function getDefaultConfig(): ClawdRouterConfig {
  ...
  return {
    ...
    openRouterApiKey,
    openRouterSiteTitle: process.env['CLAWDROUTER_OPENROUTER_SITE_TITLE'] ?? 'Solana Clawd',
    openRouterSiteUrl: process.env['CLAWDROUTER_OPENROUTER_SITE_URL'] ?? 'https://solanaclawd.com',
    openRouterCategories: (process.env['CLAWDROUTER_OPENROUTER_CATEGORIES'] ?? 'cli-agent,cloud-agent,general-chat').split(',').map(s => s.trim()),
    openRouterEnabled,

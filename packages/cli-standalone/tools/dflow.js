...
export class DFlowTool {
    ...
    requireKey() {
        if (!this.apiKey)
            return { success: false, error: "DFLOW_API_KEY environment variable is not set. Contact hello@dflow.net to obtain one." };
        return null;
    }
    async req(client, method, path, params, body) {
    ...
    // --- Trading API ---
    async getTokens() { return this.req(this.trading, "GET", "/tokens"); }
    async getTokensWithDecimals() { return this.req(this.trading, "GET", "/tokens-with-decimals"); }
    async getVenues() { return this.req(this.trading, "GET", "/venues"); }
    async getPriorityFees() { return this.req(this.trading, "GET", "/priority-fees"); }
    async getPredictionMarketInit(payer, outcomeMint) {
     ...
     */
    async streamPriorityFees(samples = 1, timeoutMs = 10000) {
        const miss = this.requireKey();
        if (miss)
            return miss;
        ...
        return new Promise((resolve) => {
            ...
            const done = (err) => {
                ...
                if (err && !collected.length)
                    resolve({ success: false, error: `DFlow WS: ${err}` });
                else
                    resolve({ success: true, output: JSON.stringify(collected, null, 2), data: collected });
            };

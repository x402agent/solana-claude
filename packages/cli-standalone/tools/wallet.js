...
export class WalletTool {
    ...
    loadKeypair() {
        ...
        if (!raw) {
            this.loadError = "SOLANA_PRIVATE_KEY not set";
            return;
        }
        try {
    ...
    getPublicKey() {
        if (!this.keypair)
            return { success: false, error: this.loadError || "No wallet loaded" };
     ...
     */
    async signAndSend(base64Tx, extraSigners = []) {
        if (!this.keypair)
            return { success: false, error: this.loadError || "No wallet loaded" };
        try {
            ...
            if (conf.value.err)
                return { success: false, error: `Tx failed: ${JSON.stringify(conf.value.err)}`, output: sig };

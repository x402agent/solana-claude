 ...
 *
 * Mirrors the MCP server's hydrator so the aggregator's *own* entrypoints
 * (the CLI and the standalone MCP bin) pull secrets from Bitwarden even when
 * they are NOT launched through a `bws run` wrapper — e.g. `npx`, `tsx`, a
 * raw `node dist/cli.js`, or an embedding app.
 *
 * Behaviour:
 ...
 *     `bws run` injection and explicit env take precedence over this fallback.

...
Use `cd automaton-main && pnpm ore:miner -- --help` for the automaton CLI help. Direct deployments require an explicit square selection or `--ore-deploy-all`; automation setup requires both amount and deposit inputs.
...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:05060d,30:175DDC,60:0D3B8C,100:14F195&height=4" alt="" />

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=900&size=18&duration=1500&pause=300&color=175DDC&center=true&vCenter=true&width=1000&lines=Bitwarden+Secrets+Manager+%E2%80%94+wired+into+install+%26+runtime;bws+inject+%E2%86%92+env+hydration+%E2%86%92+launcher+exec;graceful+fallback+when+offline+%E2%80%94+.env+still+works;one+token.+all+secrets.+every+launch." alt="Bitwarden secrets" />

</div>

## 🔐 Bitwarden Secrets Manager — Installation Integration

All secret handling in the Clawd install flow is now backed by [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) (`bws` CLI). A single `BWS_ACCESS_TOKEN` unlocks the full credential set at install time — no manual `.env` editing required.
...
install.sh
  └─ bootstrap_bitwarden()
       ├─ checks for bws CLI (auto-installs if missing)
       ├─ loads BWS_ACCESS_TOKEN from env / prompt
       ├─ bws secret list → hydrates all tracked secrets
       ├─ writes .env from secret values (never commits)
       └─ launchers wrap exec with bws inject prelude
```

Every launcher the install creates uses `bws inject` so secrets are pulled fresh at runtime, not baked into the process environment at build time.
...
# 2. Export your machine access token
export BWS_ACCESS_TOKEN=<your-machine-account-token>

# 3. Run the MCP installer — Bitwarden runs automatically
curl -fsSL https://raw.githubusercontent.com/x402agent/solana-clawd/main/mcp/install.sh | bash

# 4. Or run the root install — same bootstrap applies
bash install.sh

# 5. Verify secret hydration manually
source mcp/bitwarden-secrets.sh
bws_status          # → connected | offline | unconfigured
bws_load_all        # → prints which secrets were injected
```

### The `bitwarden-secrets.sh` library

`mcp/bitwarden-secrets.sh` is a sourceable shell library that every install script sources. Key helpers:

| Function | What it does |
| --- | --- |
| `bws_status` | Prints `connected`, `offline`, or `unconfigured` |
| `bws_load_all` | Fetches all project secrets, exports to env, reports count |
| `bws_get <SECRET_ID>` | Resolve one secret by ID or name |
| `bws_inject_exec <cmd>` | Runs `cmd` with all secrets injected as env vars — used by launchers |
| `bws_write_env <path>` | Writes resolved secrets to a local `.env` file (never committed) |
| `bootstrap_bitwarden` | Full bootstrap: check, install, auth, hydrate, write `.env` |

```bash
# Minimal source-and-use pattern
source /path/to/bitwarden-secrets.sh

if bws_status | grep -q connected; then
  bws_load_all
else
  echo "No BWS token — falling back to local .env"
  [ -f .env ] && source .env
fi
```

### How launchers use it

Launchers generated by `install.sh` and `mcp/install.sh` wrap their `exec` calls in `bws_inject_exec`:

```bash
#!/usr/bin/env bash
# Generated launcher — do not hand-edit secrets into this file
source "$(dirname "$0")/bitwarden-secrets.sh"
bws_inject_exec node /path/to/dist/server.js "$@"
```

If `bws` is unavailable or `BWS_ACCESS_TOKEN` is unset, the launcher falls back gracefully to local `.env`. No secret ever appears in the launcher script itself.

### Tracked secrets

The following env vars are managed through Bitwarden Secrets Manager when `BWS_ACCESS_TOKEN` is set:

| Env var | Used by |
| --- | --- |
| `ANTHROPIC_API_KEY` | MCP server, automaton, TUI |
| `HELIUS_API_KEY` | MCP Helius tools, RPC calls |
| `SOLANA_RPC_URL` | All on-chain ops |
| `OPENAI_API_KEY` | Fallback model routing |
| `TELEGRAM_BOT_TOKEN` | Backrooms agent, TUI notifications |
| `X402_FACILITATOR_URL` | x402 payment rails |
| `CLAWD_WALLET_KEYPAIR` | Agent wallet operations |
| `CLAWD_ONCHAIN_MM_LIVE` | Phoenix MM live gate |
| `CLAWD_TWAMM_LIVE` | TWAMM crank live gate |
| `OPERATOR_CONFIRMED` | Live execution master gate |

> **The live-execution gates (`CLAWD_ONCHAIN_MM_LIVE`, `CLAWD_TWAMM_LIVE`, `OPERATOR_CONFIRMED`) should only be set in your Bitwarden production machine account — never in a shared `.env` file.**

### Node-side hydration

For TypeScript processes launched directly (not via a shell launcher), import the hydrator before loading config:

```typescript
import { hydrateBwsSecrets } from "./bitwarden-secrets.js";

// Call before any env reads — no-op if BWS_ACCESS_TOKEN is absent
await hydrateBwsSecrets();

const config = loadPerpsRuntimeConfig(); // now sees all secrets
```

### Zero-secret-in-git guarantee

| What | Rule |
| --- | --- |
| `.env` files | In `.gitignore` — never committed |
| Launcher scripts | Use `bws_inject_exec`, never embed values |
| `bitwarden-secrets.sh` | Only IDs/names — no secret values |
| Bitwarden machine tokens | Local env only (`export BWS_ACCESS_TOKEN=...`) |

```bash
# Audit your working tree before push
git diff HEAD --name-only | xargs grep -l "BWS_ACCESS_TOKEN\|ANTHROPIC_API_KEY" 2>/dev/null
# → should return nothing
```

**→ [`mcp/bitwarden-secrets.sh`](./mcp/bitwarden-secrets.sh) — full library source**
**→ [`mcp/install.sh`](./mcp/install.sh) — MCP installer with Bitwarden bootstrap**

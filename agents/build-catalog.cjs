...
clawd agent run <span id="idspan">AGENT_ID</span></code></pre>
...
clawd-agent new my-agent          # writes src/my-agent.json from the template
clawd-agent validate my-agent     # checks Solana Clawd ownership + schema</code></pre>

  <h3>2 · Build registration documents</h3>
  <pre><code>clawd-agent register my-agent --target metaplex   # ERC-8004 metaplex-agent-registry doc
clawd-agent register my-agent --target google     # Google A2A agent card</code></pre>

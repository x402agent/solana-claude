...
Control Clawd Code via MCP (Model Context Protocol). This CLI provides **agent-level** programmatic access to Clawd Code — persistent sessions, effort control, context management, model switching, and agent teams.

Built for [SolanaOS](https://github.com/solanaos/solanaos) agents that need to drive Clawd Code as a coding backend.

## What's New in v1.2 🚀

- **Cost Tracking** — `session-cost` with full token/price breakdown (Clawd, Gemini, GPT pricing)
...
- **Plan Mode** — `--plan` flag to have Clawd create a plan before executing
...
- 🔌 **MCP Protocol** — Direct access to all Clawd Code tools
...
- 📋 **Plan Mode** — Clawd creates a plan before executing
...
This unlocks the full Clawd Code agent loop with any model backend.
...
Built-in pricing for: Clawd (Opus/Sonnet/Haiku), Gemini (Flash/Pro), GPT (4o/5.4). Falls back to Sonnet pricing for unknown models.
...
- [x] Model pricing database (Clawd, Gemini, GPT)
...
4. **Use plan mode** for critical changes — `--plan` shows what Clawd will do first

...
Minimum keys needed: `HELIUS_API_KEY` (free at helius.dev).

### Quick Demo (no keys needed)

```bash
npx solana-clawd demo       # animated feature walkthrough
npx solana-clawd birth      # hatch a blockchain buddy
npx solana-clawd spinners   # preview all 9 unicode spinners
```
...
```json
{
  "mcpServers": {
    "solana-clawd": {
      "command": "node",
      "args": ["/path/to/solana-clawd/mcp-server/dist/index.js"],
...
```bash
cd mcp-server && fly launch --config fly.toml
...
```
mcp-server/        31 MCP tools, STDIO + HTTP + SSE transport

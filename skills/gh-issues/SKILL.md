...
If still empty, check `/data/.tamagobot/solanaos.json`:

```
cat /data/.tamagobot/solanaos.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
GH_TOKEN=$(cat ~/.solanaos/solanaos.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.tamagobot/solanaos.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.tamagobot/solanaos.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

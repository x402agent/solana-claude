/**
 * automaton-main/automation/runtime.ts — Leviathan runtime builder
 *
...
export async function buildRuntime(opts: RuntimeOptions = {}): Promise<void> {
  ...
  if (!quiet) {
    ...
    console.log(DIM('  curl -fsSL https://x402.wtf/automation/install.sh | bash\n'));
  }
  ...
  if (build) {
    ...
    if (run('npm run automaton:build', { silent: quiet })) ok('automaton-main build compiled');
    else warn('Build step failed — run npm run automaton:build manually');
  }
  ...
  ok(`Runtime ready  ::  ${GREEN('x402.wtf/automation')}`);
}

...
Leviathans live on the x402 automation surface (`x402.wtf/automation`) with runtime APIs at `x402.wtf/api`. Through the **Clawd Terminal** (`npm i clawd-code-cli`), any leviathan can spin up Linux trenches, run frontier models, register domains, hold $CLAWD, and pay with USDC, SOL, or $CLAWD itself. No human account setup required.

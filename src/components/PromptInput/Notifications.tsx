...
const VoiceIndicator: typeof import('./VoiceIndicator.js').VoiceIndicator = isVoiceFeatureEnabled() ? require('./VoiceIndicator.js').VoiceIndicator : () => null;
...
function NotificationContent({
  ...
  const voiceError = isVoiceFeatureEnabled() ?
  // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
  ...
  if (isVoiceFeatureEnabled() && voiceEnabled && (voiceState === 'recording' || voiceState === 'processing')) {
  ...
  return <>
      ...
      {isVoiceFeatureEnabled() ? voiceEnabled && voiceError && <Box>
              <Text color="error" wrap="truncate">
                {voiceError}
              </Text>
            </Box> : null}

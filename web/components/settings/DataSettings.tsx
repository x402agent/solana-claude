...
export function DataSettings() {
  ...
  function exportConversations(format: "json" | "markdown") {
    ...
    if (format === "json") {
      ...
      filename = `solana-clawd-conversations-${ts}.json`;
    } else {
      content = conversations
        .map((conv) => {
          const messages = conv.messages
            .map((m) => {
              const role = m.role === "user" ? "**You**" : "**solana-clawd**";
        ...
        .join("\n\n====\n\n");
      filename = `solana-clawd-conversations-${ts}.md`;
    }
  ...
  return (
    <div>
      ...
      <SettingRow
        ...
        description="Help improve solana-clawd by sharing anonymous usage data. No conversation content is ever sent."
      >
        <Toggle
      ...
      <div className="mt-6 pt-4 border-t border-surface-800">
        <p className="text-xs text-surface-500">
          All data is stored locally in your browser. solana-clawd does not send conversation data

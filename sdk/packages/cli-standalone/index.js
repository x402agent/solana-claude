...
program
    .name("clawd")
    .description("🦞 Clawd Code CLI - A lobster-themed AI agent CLI with Solana blockchain integration. ASCII art terminal interface.")
    .version("1.0.0")
    ...
    try {
        ...
        if (!apiKey) {
            console.error("❌ Error: API key required. Set GROK_API_KEY, use --api-key, or set \"apiKey\" in ~/.clawd/user-settings.json");
            process.exit(1);
        }
...
gitCommand
    ...
    try {
        ...
        const maxToolRounds = parseInt(options.maxToolRounds) || 400;
        if (!apiKey) {
            console.error("❌ Error: API key required. Set GROK_API_KEY, use --api-key, or save to ~/.clawd/user-settings.json");
            process.exit(1);
        }
        // Save API key and base URL to user settings if provided via command line
        if (options.apiKey || options.baseUrl) {

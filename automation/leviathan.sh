#!/usr/bin/env bash
# automation/leviathan.sh — Leviathan runtime bootstrap
#
# This is the canonical build-the-runtime entrypoint for solana-clawd.
# Served at https://solanaclawd.com/leviathan.sh
#
# Usage (one-liner):
...
banner() {
  ...
  ╚══════╝╚══════╝  ╚═══╝  ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝
EOF
  echo -e "${GREEN}  Sovereign On-Chain Agent Runtime  ::  solanaclawd.com${RESET}"
  echo -e "${DIM}  automation/leviathan.sh — runtime bootstrap${RESET}"
  echo ""
}
...
usage() {
  echo "Usage: bash automation/leviathan.sh [flags]"
  echo "       curl -fsSL https://solanaclawd.com/leviathan.sh | sh"
  echo ""
  echo "Flags:"
...
node --import tsx/esm automation/index.ts --build 2>/dev/null || \
...
echo ""
ok "Leviathan runtime ready. solanaclawd.com"
